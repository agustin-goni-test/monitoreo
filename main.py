...
#         # Get access to the openpyxl worksheet
#         workbook = writer.book
#         worksheet = writer.sheets["Stats"]

#         # Set column widths
#         worksheet.column_dimensions['A'].width = 50  # Metric
#         worksheet.column_dimensions['B'].width = 25  # Value

#         # Apply number formatting and alignment
#         for row in worksheet.iter_rows(min_row=2, max_col=2, max_row=7):  # rows with data
#             metric_cell, value_cell = row
#             if "Percent" in metric_cell.value:
#                 value_cell.number_format = '0.00%'  # Format as percentage
#             else:
#                 value_cell.number_format = '#,##0.0'  # Format with thousands separator and 1 decimal

#             # Center-align all cells in column B
...
def output_to_excel(service_name, response_data):
    """
    Create an Excel report with raw data, statistics, daily aggregates, and pivot analysis.
    """
    series = response_data['result'][0]['data'][0]
    timestamps = series['timestamps']
    values = series['values']
